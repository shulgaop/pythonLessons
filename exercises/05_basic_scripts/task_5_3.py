...
mode=input('введите рижим работы интерфейса(access/trunk):')
interface=input('введите тип и номер интерфейса(Gi0/1):')
vlan=input('введите номер VLANа (для режима trunk будет вводиться список VLANов):')
#поправлю темплейты
access_template = [
    "switchport mode access",
